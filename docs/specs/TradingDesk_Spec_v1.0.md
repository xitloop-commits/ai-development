...
### 3.1 Tabs
- **My Trades LIVE**: Active by default, indicated by a cyan border and a green "LIVE" badge.
- **AI Trades PAPER**: Indicated by a yellow "PAPER" badge.

### 3.2 Summary Bar
A single-row sticky bar providing an at-a-glance snapshot of the active tab's status:
- **Current Day**: E.g., "7 of 250 days".
- **Trade Capital**: The starting capital for the current Day Index.
- **Available**: Capital currently free for new trades.
- **Profit**: Cumulative net profit from the Trading Pool since Day 1.
...
- **Rating**: Based on profit percentage against Trade Capital (e.g., 🏆 for ≥5%, 🏆🏆 for ≥10%).
...
- **New Trade Input Row**: Slides in when the (+) button is clicked. Contains inputs for Instrument, B/S, CE/PE, Strike, Entry (auto-filled with LTP but editable), and Capital % (5%–100% of Available Capital).
- **Summary Row**: Aggregates all trades for the day. Shows total Qty, total P&L, live Actual Capital, and live Deviation.
