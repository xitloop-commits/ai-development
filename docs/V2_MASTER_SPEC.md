...
**Probability calibration (LOCKED 2026-05-17 — D72, ChatGPT feedback):** raw LightGBM `predict_proba` values are NOT real probabilities — a model output of 0.70 may correspond to actual 55% win rate. Gate logic that treats raw scores as probabilities will mis-fire systematically. Calibration fixes this:
...
| 7 | **Calibration map presence (LOCKED 2026-05-17 — D72)** | For each of 84 heads × 4 instruments, verify `<head>.calibration.json` exists and parses. Missing or corrupt = RED (head would predict uncalibrated, gate thresholds wrong). Wave 2 heads (pre-calibration) ship with identity-map calibration files for compatibility — see migration note in D72 |
...
| Sat 21:30 | **Probability calibration fit (LOCKED 2026-05-17 — D72)** | For each candidate head, fit isotonic calibration on the dedicated 1-week calibration fold; write `<head>.calibration.json` alongside `.lgbm`. ~5 min × 4 instruments. Calibration uses CANDIDATE head's raw predictions on the calibration fold vs realized targets |
