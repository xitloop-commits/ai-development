...
**Proactive auto-pay reminder.** Each account's Data API plan auto-renews monthly on a fixed day via a bank auto-pay; if the bank balance is short the auto-debit fails and the plan lapses (the 1006 loop above). To prevent that, Lubas warns ahead of each renewal. Renewal dates live in `config/subscriptions.json` (`renewalDayOfMonth` + `autopayBank` per account; `leadDays` window — currently primary = 30th, secondary = 24th, lead 5 days, bank ICICI; edit this file when a date changes, no code change needed). For the `leadDays` days up to and including each renewal day, two surfaces fire:
- **API console** — `startSubscriptionAlertScheduler()` (in `server/_core/subscriptionAlert.ts`, wired from `server/_core/index.ts`) logs a WARN on server startup and every 12h while in any account's window.
- **yow-partha Telegram bot** — the same reminder is pushed via `notifyPartha()` (`server/_core/telegram.ts`, YOW_PARTHA_* channel), de-duped to once per account per calendar day via `data/subscription-alert-state.json`.

Weekend/holiday-proofing: the Windows task **`Lubas-SubscriptionAlert-Daily`** (registered by `startup/install-scheduled-tasks.ps1`, or standalone `startup/register-subscription-alert-task.ps1`) runs `startup/subscription-alert.bat` → `scripts/subscription-alert.ts` daily at 09:00 even when the API server is down. The shared per-day de-dup state means the task and the in-server scheduler never double-send. Date-window logic is covered by `server/_core/subscriptionAlert.test.ts` (incl. month-end clamping, e.g. a 30th renewal → Feb 28).
