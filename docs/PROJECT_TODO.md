...
- **Follow-ups:** (1) the **11 already-stuck PENDING** testing-live trades won't self-heal (Dhan won't re-push terminal events) — need a one-shot reconcile or manual cleanup. (2) `getOpenPositions` filters `status:"OPEN"`, so the recovery engine is **blind to PENDING** ([storage.ts:389](server/portfolio/storage.ts#L389)) — irrelevant now that the WS works, but if we keep any backstop it must query PENDING. (3) testing-live appends to a **stale `2026-06-19` day record** (rollover bug). (4) REST `DHAN_ORDER_STATUS_MAP` still keys on UPPERCASE — verify REST order-status casing if we ever reconcile via the order book.
