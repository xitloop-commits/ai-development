...
### T96 [CAPITAL · P0] — Clawback re-fires on every trade close, draining the pool — FOUND 2026-07-20 🆕🔴

`completeOrClawbackSingle` (`portfolioAgent.ts:1351`) runs after EVERY trade close and fires whenever the day's RUNNING loss exceeds the target:

```
if (day.totalPnl < 0 && Math.abs(day.totalPnl) >= day.targetAmount) {
  const clawback = processClawback(day.totalPnl, state);   // newPool = pool − |loss|
```

Nothing marks the day as already clawed back, so once the day dips past the target EVERY subsequent close debits the pool by the ENTIRE running loss again. Demonstrated: a day that lost ₹7,000 debits ₹36,100 across six closes.

**Observed on paper 2026-07-20:** pool 69,011 against a day-1 tradeCapital of 100,000 and a day loss of only 1,916 — a −30,989 phantom drain. `profitHistory` empty and day 1 still ACTIVE, so no completion could have moved it. True position was 98,083.81; net worth understated by ~29k.

Also note `processClawback` debits `tradingPool` by the loss even when `profitHistory` is EMPTY — i.e. it "claws back" profit that was never booked, taking it out of seed capital. Worth deciding whether a clawback with nothing to consume should be a no-op.

Fix: make the clawback idempotent per day — record what has already been clawed back on the day record (or reset the day's running P&L when it fires) so a second close can't re-debit. Regression test must close several trades on an already-clawed-back day and assert the pool moves once.

**Related, found in the same audit:**
- `resetCapital` zeroes `cumulativePnl`/`cumulativeCharges` mid-day while the day record keeps all its trades, so the counters permanently disagree with the day. Decide whether a reset should also clear the day record.
- `replaceCapitalState` uses `$set`, so a reset does NOT clear `peakCapital` (observed stale at 1,940,930 after a reset to 100,000 — every drawdown % is measured against a peak that no longer exists) or `seededAt` (harmless on paper; on a LIVE book it means a reset leaves it marked seeded and it never re-reads Dhan).
