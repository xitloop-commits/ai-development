...
**Phase 2+ (deferred):** per-instrument toggles; confirm-guard before turning **scalp** off (halts all entries); a gate-mode switch (current/wave1/wave2/legstart); cross-client live panel sync by consuming the `sea_control` ws broadcast the server already emits.
