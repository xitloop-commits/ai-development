...
- **DEFERRED (needs a call):** "manual drag OVERRIDES the engine on Runway/Anchor." Today a manual SL/TP drag on a Runway/Anchor twin is reclaimed by the engine on the next tick (it re-derives the stop every tick). Honoring override-wins needs a per-trade override flag the engine yields to — a server change to the autonomous engine. Left out so tomorrow's race runs clean/untouched; add after one confirmed race day. (Sprint drags already persist fully.)
