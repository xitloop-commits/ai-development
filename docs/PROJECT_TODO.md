...
- **(1) Detected-not-completed** — trade #43 (loser). `exitReason=SL_HIT` **stamped** but `status=OPEN`, no fill, P&L unrealised. Exit was *detected*; the close never finalised. Hypothesis: tickHandler stamps `exitReason`, adds to the in-memory `exitingTrades` guard + pushes to `tradesToExit`, but `executor.exitTrade` → status→CLOSED + fill + realised P&L fails/interrupts; the guard (and already-set `exitReason`) then **blocks retry** → stuck forever though ticks keep coming and the exit test is still true.
- **(2) Never-detected** — trades #52/#57 (Anchor winners, NIFTY50 24250 CE, ids T1784270104868-qw57cy / T1784270705771-e8l3wg). `exitReason=null`, `status=OPEN`, `targetPrice=122.99/123.44` (real), `ltp=peakLtp=171.95` → target-bank test `171.95 ≥ 122.99` TRUE, yet the bank **never fired at all**. Tell-tale: `peakLtp == ltp` exactly (no intermediate high) + `lastTickAt=today` → the price appears to be refreshed by **a path that updates `ltp`/`peakLtp`/`lastTickAt` but does NOT run the exit engine** (`decideExit`/dispatch), so the exit condition is never evaluated. Find that price-update path and make it (or a sweep) run the exit checks.
