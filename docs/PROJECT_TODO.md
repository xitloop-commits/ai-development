...
- **OPEN QUESTION (MA-Signal ride vs strategy-driven):** MA-Signal trades still OPEN with `manualExitOnly`/SL-off/TP-off (`tradeExecutor.ts:1225-1231`), so the **Sprint** MA twin still rides to the MA reversal with TP/TSL frozen (what Partha saw). Runway/Anchor MA twins are engine-driven (dispatch runs before the master skip) so they DO trail. To make "only the strategy drives" fully true for MA-Signal, drop those open-time defaults so Sprint runs TP/SL/TSL + still exits on the MA reversal (matches the spec line "Sprint twin exits on MA EXIT + its TP/SL/age"). Awaiting Partha's call.
