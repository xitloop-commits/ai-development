...
**Sub-phases (sequential — each its own PR, each golden-file gated). B.3 list locked 2026-05-25 against profile data on 2026-04-28 (pre-v8) + 2026-05-22 (v8 schema) — both rankings agreed.**
  ...
  - [~] **B.3c — `active_features.py` cluster → columnar** (PARTIAL 2026-05-27): `compute_side_strengths` cache infrastructure built (scalar-based memoization via `max_pain_cache.install_side_strengths` + `TFA_LEGACY_SIDE_STRENGTHS=1` rollback) but **install DISABLED by default** — scalar-cache approach measured neutral on wall time (build ~5.6s offline ≈ runtime savings ~5s amortisation, net within ±0.05× noise). Needs proper Polars `compute_side_strengths_batch` (shift-over-strike + group_by-normalize within snapshot) to actually win; that follow-up plugs into the existing wrapper API. `_c7_center_of_mass` + `compute_strike_rotation_features` not attempted (history-deque based, separate design pass).
  - [ ] **B.3d — `dealer_hedging` + `greeks.bs_greeks` cluster → columnar** (deferred 2026-05-27): Same complexity profile as B.3c — per-emit spot dependency inside `bs_greeks` means scalar caching can't win. Needs full Polars per-emit × per-strike Black-Scholes broadcast. Defer to a Polars-vectorisation session.
