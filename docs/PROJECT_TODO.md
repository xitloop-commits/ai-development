...
**PENDING (chunk 2b + 5):** wire the remaining apply-paths per-mode with the AI-only guard — Sprint trailing (tickHandler), sizing/aiLiveLotCap (entry), RCA age/stale/vol (RcaMonitor), EOD square-off (scheduler); then strip the Order Execution section + shrink Trade Executor to RCA-channels in `client/src/pages/Settings.tsx`. Subtlety to honour: square-off/RCA also touch `my-live` → AI channels use the per-mode config, `my-live` keeps executor-settings defaults.
