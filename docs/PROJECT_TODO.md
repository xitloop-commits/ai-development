...
- **STATUS 2026-07-18 — Phase 1a/1b/1c DONE + pushed (`fb55ace`, `76d5b5f`):** adapter `onTick` now emits to `tickBus` unconditionally (kills the single-slot clobber; dead `tickCallback` removed); no-op `feed/subscribe` neutralised; all channels (incl. ai-live) read ticks from the primary `getActiveBroker()` (orphaned adapter/channel params cleaned). Typecheck clean; tick-path + 26/26 executor tests green (pre-existing integration + dhan-auth failures unrelated). **Phase 1d (retire #3) DEFERRED** — it's an optimisation (α already fixed), it's TFA-adjacent (secondary account), and it needs live verification. Do it at market open with a rollback. **α fix itself needs a live-feed smoke test at next market open** (repro: open trade + Python feed/subscribe → confirm ticks keep flowing to the engine).
