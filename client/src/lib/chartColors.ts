 ...
 */
export type ChartTheme = 'light' | 'dark' | 'slate';
...
export function chartColors(theme: ChartTheme): ChartColors {
  ...
  if (theme === 'slate') {
    // Navy/slate dark theme — matches the app's slate palette.
    return {
      background: '#0b1120',
      text: '#94a3b8',
  ...
  // dark (Obsidian Glow) — values match the legacy CHART_* constants.
  return {
    background: '#131722',
    text: '#94a3b8',
    grid: 'rgba(148,163,184,0.06)',
