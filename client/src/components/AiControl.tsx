/**
 * AiControl â€” the single AI menu (merges the old SEA control + AI-trades mode).
 *
 * The "AI" CTA (now on the desk header, beside the trade filter) opens a control
 * panel for the AI stream of the CURRENT BOOK â€” it follows the app-bar Paper/Live
 ...
 *
 * MANUAL ("My Trades") settings are NOT here â€” they moved to their own AppBar
 ...
 *
 * Sections: AI-trades switch Â· cohorts Â· strategies (N on = N trades/signal) Â·
 * model Â· sizing Â· order Â· Sprint / Runway / Anchor / Glide exit configs. The
...
/** "percent" = % of premium; "rupees" = net â‚¹ P&L (after charges) on the position. */
...
/** Glide has no trading levels â€” only the disaster stop. See GlideConfig. */
interface GlideCfg {
  disasterSlPct: number; giveBackArmPct: number; giveBackPct: number;
  tpEnabled: boolean; tpMode: ExitLevelMode; tp: number;
}
/** T147 â€” Ladder: MSL/SL/TSL/MTP knobs (mirrors server LadderConfig). */
interface LadderCfg {
...
interface ModeCfg {
  ...
  /** T144 â€” per-cohort strategy race: each cohort's enabled strategies (one
...
/** T129 â€” system-wide settings; edited in the Settings menu, not here. */
...
/** T134 â€” each book carries its OWN strategy exits + an AI and a manual stream. */
 ...
 *
 * One entry per SETTING, never per strategy â€” Runway and Anchor share Cooling /
...
/**
 * An SL/TP row with a % / â‚¹ mode toggle. In "%" the number is a premium %; in
 * "â‚¹" it is a NET rupee P&L (after charges) on the whole position â€” the engine
  ...
  return (
    <Row label={label} help={help}>
      <div className="flex items-center gap-1">
        ...
        />
        <span className="text-[0.5625rem] text-muted-foreground w-6">{rs ? "â‚¹" : "%"}</span>
      </div>
    </Row>
  );
}
...
/**
 * An ES-honour CAP row: an ON/OFF toggle, then â€” right in front of the input â€”
 * a SINGLE button that flips the unit % â†” â‚¹ (one control, both actions), then
...
function Group({ title, children, help, collapsible = false }: {
  ...
  // T130 â€” the four strategy-exit groups open COLLAPSED: their knobs are tuned
 ...
 *  MSL / SL / TSL / TTP / MTP blocks) â€” a hairline rule + a small caption. */
 ...
 * `breakevenAt` / `target` are defined once here and referenced from both â€” the
 * text is never duplicated per group.
 */
const HELP = {
  model:
    "Which trained model version the running SEA scores signals with. Switching applies immediately â€” SEA swaps the model and its feature preprocessor together at the next tick, no restart. It also becomes the startup default. Each option shows its mean AUC and head count; AUC is only comparable between versions with the SAME head count, since it averages over whichever heads that version trained.",

  // Strategy-level: what the whole strategy does.
  glide:
    "MA-Signal only. No stop, no target, no trailing â€” the trade rides until MA-Signal sends its own EXIT. Two safety nets sit under it: a wide disaster stop for the case where that exit never arrives, and a give-back guard for the case where it arrives too late.",
  glideTp:
    "Optional hard take-profit for Glide (off by default â€” Glide normally rides to the MA EXIT). On = also bank a target: % of premium, or a NET â‚¹ profit after charges. Off = pure Glide, no target.",
  glideDisaster:
    "Last line of defence, NOT a trading stop. Only fires if the MA EXIT never comes at all (SEA restarted and lost its leg map, or a manual Glide trade was forgotten). Keep it wide enough that ordinary MA behaviour never reaches it â€” a 40% swing in an option premium is normal inside one leg.",
  glideArm:
    "How far up a trade must go, as a % of entry, before the give-back guard starts watching it. Below this the trade is left completely alone and rides to the MA EXIT â€” which is what keeps this from being a stop-loss.",
  glideGiveBack:
    "Once armed, close the trade if it hands back this much of its BEST profit. 50% means: up 30 points at the peak, exit if it falls back to 15. Set to 0 to switch the guard off and get pure Glide. Measured 22-23 Jul: Glide reached â‚¹4.6L of peak profit and booked â‚¹1.3L â€” 69% given back, six winners finishing as losses.",

  sprint:
    "Simplest strategy. Sets a fixed stop and target at entry from the percentages below, then trails the stop up behind the running peak. No staged phases â€” the stop starts where you set it and only ever ratchets in your favour.",
  runway:
  ...
  // Sprint-only.
  sprintSL: "Stop-loss. % = a % move below entry premium. â‚¹ = a NET rupee loss (after round-trip charges) on the whole position â€” exits the moment live net P&L drops to âˆ’â‚¹ this, whatever the lot size.",
  sprintTP: "Take-profit. % = a % move above entry premium. â‚¹ = a NET rupee profit (after round-trip charges) on the whole position â€” banks the moment live net P&L reaches +â‚¹ this.",
  dailyTarget:
  ...
  sprintTrailPct:
    "Gap kept below the running peak, as a % of the peak. The stop trails from the FIRST tick and only ratchets up â€” it never crawls back down. Note: if you set this tighter than the opening stop-loss above, the stop jumps up immediately at entry.",
  trailFrom:
  ...
  activationGate:
    "How far past breakeven price must go before the trailing stop is armed. LIVE ONLY â€” this arms the broker's native trailing on a Dhan Super Order. It has no effect on paper trades, which trail from the first tick regardless.",
  activationHold:
  ...
  // Shared by Runway AND Anchor â€” written once, used twice.
  cooling:
  ...
  wideStop:
    "The stop during the cooling window, as a % below entry. Deliberately loose â€” it's there so the trade is never naked, not to be hit. Switch to â‚¹ to make it a FLAT net-rupee stop (after charges) instead â€” the staged tightening/cooling then no longer applies.",
  cooledStop:
    "The tighter stop that replaces the wide one once cooling ends, as a % below entry.",
  breakevenAt:
    "Once the peak reaches this fraction of the target gain, the stop moves up to your entry price â€” from that point the trade can't lose. 0.5 = halfway to target.",
  target:
    "Target gain as a % of entry. This is the ONLY source of the target: the signal's own target is ignored, so changing this moves the target on open trades too. Switch to â‚¹ for a NET rupee profit target (after charges) on the whole position instead.",
  ...
  runwayTrailPct:
    "Once trailing is active, the stop sits this % below the running peak â€” with a floor at half the target gain, so a winner can't give everything back.",

  // Ladder (T147) â€” cut losers, ride winners. A stepped SL + trailing TSL + Ã—R target.
  ladder:
  ...
  ladderMslPct:
    "How far below entry the safety-net floor sits, as a %. It sits wider than the SL start â€” the stop normally closes the trade first; this only matters on a violent gap.",
  ladderSlMode:
    "Current = the staged SL that tightens over time + the TSL that arms once you're in profit. Fixed = a classical flat stop at 'Fixed SL' below entry that never moves (no stepping, no TSL) â€” the trade exits there or at MTP.",
  ladderSlFixed:
    "The fixed classical stop, as a % below entry. It never moves â€” the trade rides until price hits it or reaches MTP.",
  ladderSlStart:
    "Where the stop opens, as a % below entry. This is also the 'risk' the target multiplies â€” a 5% start with a 2Ã— target aims for a 10% gain.",
  ladderSlFloor:
    "The tightest the stepping stop can get, as a % from entry. It steps in from 'SL start' toward this and stops there â€” never tighter.",
  ladderSlStep:
  ...
  ladderSlGap:
    "Self-close guard. The stop is never tightened to within this % of the LIVE price â€” if a step would move it that close it HOLDS instead, so the stop can't rise into the price and close the trade on its own.",
  ladderTslArm:
  ...
  ladderTslPct:
    "The trailing distance â€” a % below the peak ('peak' mode), or the % of the peak gain handed back ('giveback' mode).",
  ladderTtpStart:
    "Where the trailing take-profit LINE starts, as a % above entry. TTP is VISUAL ONLY â€” it never exits (MTP is the exit). It marks 'where the ride is pointing'.",
  ladderTtpTrail:
    "Once price runs up, the TTP line floats this % ABOVE the running high â€” as new highs print, it climbs to keep the gap. It never drops. Drawn at max(entry+start%, peak+trail%).",
  ladderMtpR:
    "The take-profit exit (MTP). Basis Ã—risk = a multiple of the initial risk ('SL start'), so 2Ã— with a 5% start banks at +10%. Basis % = a plain % above entry you type directly (e.g. 25 = bank at +25%), independent of the SL.",
  ladderEsHonour:
    "Whether the trade exits on SEA's exit signal ONLY. Off (default) = the signal is visual-only and the ladder's own SL/TSL/MSL/MTP run the trade. On = the ladder's own TSL/MSL/MTP are disabled and the trade rides until the model's exit signal fires â€” except for the safety SL below.",
  ladderEsSl:
    "The one hard stop kept while riding to the exit signal (ES-honour ON). Basis %: a flat % below entry. Basis â‚¹: a gross rupee loss (converted to a price via the position size). Its marker is drawn on the bar.",
  ladderEsMtp:
...
export function AiControl({ replay = false }: { replay?: boolean } = {}) {
  const [open, setOpen] = useState(false);
  // T131 â€” the menu follows the app-bar Paper/Live tab; it has no toggle of its
  ...
  // T137 â€” a replay-settings instance edits the `replay` block; the normal AI
  ...
  // Trained model versions on disk. Static between retrains, so no polling â€”
  ...
  // the per-mode block) when the mode changes â€” deliberately NOT on every `all`
  ...
  // Another panel applied a change â†’ refetch so `dirty` compares against the
  ...
  // T134 â€” ONE Apply for the whole panel (cohorts/strategies/sizing/order AND
  // strategy exits), both for the current book. Sequential await so the exits
  // response â€” which reflects the block change already persisted â€” is the final
  ...
  // T130 â€” the LABEL colour is the liveness indicator; no separate dot.
  ...
  return (
    <div className="relative shrink-0 self-stretch flex" ref={ref}>
      <button
        ...
        title={replay
          ? "Replay settings â€” cohorts, strategies, sizing, exits used ONLY during a replay run"
          : "AI trades â€” cohorts, strategies, sizing, exits for the current book"}
      >
        {replay
      ...
      {open && (
        <>
          <div className="absolute right-0 top-full mt-1 z-50 w-80 rounded-md border border-border bg-popover text-popover-foreground shadow-xl">
            {/* Panel title â€” the book comes from the app-bar tab. */}
            ...
            {/* â‘  AI trades switch â€” normal AI menu only. A replay never places on
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[0.5625rem] text-muted-foreground">
                  {aiOnForMode
                    ...
                    : "signals still logged â€” nothing placed on this book"}
                </span>
              </div>
            ...
            {!d ? (
              <div className="p-6 text-center text-[0.625rem] text-muted-foreground">Loadingâ€¦</div>
            ) : (
              <>
                ...
                {/* â‘¡b Model â€” which trained version the RUNNING SEA scores with.
                    Applies immediately (hot-swap, no restart) and is NOT part of
                    the paper/live draft: there is one SEA process, so the model
                    is global, not per-mode. Hidden in Replay settings â€” a replay
                        ...
                        >
                          ...
                          {list.map((m) => (
                            <option key={m.version} value={m.version}>
                              {m.version}
                              {m.auc != null ? ` Â· auc ${m.auc}` : ""}
                              {m.heads != null ? ` Â· ${m.heads}h` : ""}
                            </option>
                          ))}
                        </select>
                      </Row>
                ...
                {/* â‘¢ Strategy RACE per cohort (T144). For each cohort that's on,
                    toggle which exit strategies to run â€” a signal places ONE trade
                ...
                <div className="border-t border-border pt-2 flex flex-col gap-1.5">
                  <span className="flex items-center gap-1.5">
                    ...
                    <InfoDot text="For each cohort, pick which exit strategies to run. A signal places ONE trade per enabled strategy â€” so Sprint / Runway / Anchor race on the SAME signal and you can see which handles that cohort best. The cohort's default (Settings â†’ Cohort strategies) is locked ON so a cohort is never silenced. Glide is MA-only." />
                  </span>
                  <div className="flex flex-col gap-1">
                    ...
                    {!COHORTS.some((c) => d.cohorts[c.key]) && (
                      <span className="text-[0.5625rem] text-muted-foreground">No cohorts on â€” nothing will trade.</span>
                    )}
                  </div>
                </div>
                ...
                {/* Strategy exits â€” PER BOOK (T134). Paper and live can be tuned
                ...
                <div className="border-t-2 border-warning-amber/30 pt-2 mt-1 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-1.5">
                      <SectionLabel><span className="text-warning-amber">Strategy exits</span> Â· {replay ? "REPLAY" : mode === "live" ? "LIVE" : "PAPER"}</SectionLabel>
                      <InfoDot text="Per book â€” paper and live each have their own exit tuning, so you can test on paper without changing live. Both hand-placed and AI trades on this book use these." />
                    </span>
                  ...
                  <Group title="Runway" help={HELP.runway} collapsible>
                    ...
                    <Num help={HELP.breakevenAt} label="Breakeven at" value={ed.runway.breakevenAtFrac} step={0.05} min={0} max={1} unit="Ã—" onChange={(v) => editExits((x) => { x.runway.breakevenAtFrac = v; })} />
                    <Num help={HELP.trailAt} label="Trail at" value={ed.runway.nearTargetFrac} step={0.05} min={0} max={1} unit="Ã—" onChange={(v) => editExits((x) => { x.runway.nearTargetFrac = v; })} />
                  ...
                  <Group title="Anchor" help={HELP.anchor} collapsible>
                    ...
                    <Num help={HELP.breakevenAt} label="Breakeven at" value={ed.anchor.breakevenAtFrac} step={0.05} min={0} max={1} unit="Ã—" onChange={(v) => editExits((x) => { x.anchor.breakevenAtFrac = v; })} />
                  ...
                  <Group title="Glide" help={HELP.glide} collapsible>
                    ...
                    {ed.glide.giveBackPct === 0 && (
                      <span className="text-[0.5625rem] text-warning-amber leading-snug">
                        Give-back guard OFF â€” a Glide trade will hand back the whole
                  ...
                  <Group title="Ladder" help={HELP.ladder} collapsible>
                    {/* ES honour at the TOP â€” when ON, the ladder's own exits are
                        OFF and everything below is inert (rides to the signal). */}
                    <SubGroup>ES Â· model exit signal</SubGroup>
                    ...
                    {ed.ladder.esHonour ? (
                      ...
                      <>
                        <span className="text-[0.5625rem] text-muted-foreground leading-snug">
                          OFF â€” the exit-signal marker is visual-only; the ladder's own stops run the trade.
                        </span>
                        <SubGroup>MSL Â· safety net</SubGroup>
                        ...
                        <SubGroup>SL Â· stop mode</SubGroup>
                        ...
                        {ed.ladder.slMode === "fixed" ? (
                          ...
                          <>
                            ...
                            <SubGroup>TSL Â· trailing stop</SubGroup>
                        ...
                        <SubGroup>TTP Â· trailing profit (visual)</SubGroup>
                        ...
                        <SubGroup>MTP Â· take-profit exit</SubGroup>
                        <Row label="MTP (Max TP) basis" help={HELP.ladderMtpR}>
                          <div className="flex gap-1">
                            <Pill label="Ã—risk" on={ed.ladder.mtpMode === "R"} onClick={() => editExits((x) => { x.ladder.mtpMode = "R"; })} />
                            <Pill label="%" on={ed.ladder.mtpMode === "percent"} onClick={() => editExits((x) => { x.ladder.mtpMode = "percent"; })} />
                          </div>
                        </Row>
                        {ed.ladder.mtpMode === "R" ? (
                          <Num help={HELP.ladderMtpR} label="MTP Ã—risk" value={ed.ladder.mtpR} step={0.5} min={1} max={10} unit="Ã—" onChange={(v) => editExits((x) => { x.ladder.mtpR = v; })} />
                        ) : (
