...
const placeTradeUiSchema = z.object({
   ...
   */
  exitStrategy: z.enum(["sprint", "runway", "anchor", "glide", "ladder", "rider"]).optional(),
});
...
export const executorRouter = router({
  ...
  placeTrade: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const aiLevels = sprintOpeningLevels(input.channel, entryPrice, isBuy, qty);
  ...
  setExitStrategy: protectedProcedure
    .input(z.object({
      channel: channelSchema,
      tradeId: z.string().min(1),
      exitStrategy: z.enum(["sprint", "runway", "anchor", "glide", "ladder", "rider"]),
    }))
    .mutation(async ({ input }) => {
      const day = await portfolioAgent.ensureCurrentDay(input.channel);
