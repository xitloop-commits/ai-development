...
export interface PositionStateDoc {
  ...
  /** T171 — collapsed to one exit ("rider"); free string so old docs keep their label. */
  exitStrategy?: string;
  ...
  /** MOST-ADVERSE LTP seen since entry — the mirror of peakLtp (lowest for a BUY,
...
function docToPositionState(doc: Record<string, any>): PositionStateDoc {
  return {
    ...
    exitStrategy: doc.exitStrategy ?? "rider",
