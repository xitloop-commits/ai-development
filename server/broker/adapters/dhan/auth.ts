 ...
 *
 * Marks the broker token as expired in MongoDB and emits a Telegram alert.
 * Does NOT auto-refresh — the policy (May 6 2026) is: refresh only at server
 * startup, never mid-session. Operator must restart BSA to mint a fresh token.
 */
...
export async function handleDhan401(brokerId: string): Promise<null> {
  ...
  await notifyTelegram(
    `⚠️ <b>Dhan token expired mid-session</b>\n` +
