...
const brokerSettingsSchema = z.object({
  ...
  slMode: z.enum(["percent", "fixed"]).optional(),
  targetMode: z.enum(["percent", "fixed"]).optional(),
  slFixedOptions: z.number().min(0).max(100000).optional(),
  slFixedOther: z.number().min(0).max(100000).optional(),
  tradeTargetOptionsFixed: z.number().min(0).max(100000).optional(),
  tradeTargetOtherFixed: z.number().min(0).max(100000).optional(),
