...
const settingsSchema = new Schema<BrokerSettings>(
  {
    ...
    // Fixed (₹/points) risk values + per-block mode selector. "percent" (default)
    // keeps the %-of-entry behaviour; "fixed" uses these absolute distances.
    slMode: { type: String, enum: ["percent", "fixed"], default: "percent" },
    targetMode: { type: String, enum: ["percent", "fixed"], default: "percent" },
    slFixedOptions: { type: Number, default: 10 },
    slFixedOther: { type: Number, default: 5 },
    tradeTargetOptionsFixed: { type: Number, default: 40 },
    tradeTargetOtherFixed: { type: Number, default: 5 },
    trailingStopEnabled: { type: Boolean, default: true },
    trailingStopPercent: { type: Number, default: 2.0 },
    trailingDistanceSource: { type: String, enum: ["config", "signal"], default: "signal" },
...
function docToConfig(doc: Record<string, any>): BrokerConfigDoc {
  return {
    ...
    settings: {
      ...
      slMode: doc.settings?.slMode ?? "percent",
      targetMode: doc.settings?.targetMode ?? "percent",
      slFixedOptions: doc.settings?.slFixedOptions ?? 10,
      slFixedOther: doc.settings?.slFixedOther ?? 5,
      tradeTargetOptionsFixed: doc.settings?.tradeTargetOptionsFixed ?? 40,
      tradeTargetOtherFixed: doc.settings?.tradeTargetOtherFixed ?? 5,
      trailingStopEnabled: doc.settings?.trailingStopEnabled ?? true,
      trailingStopPercent: doc.settings?.trailingStopPercent ?? 2.0,
      trailingDistanceSource: doc.settings?.trailingDistanceSource ?? "signal",
