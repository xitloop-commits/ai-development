...
while True:
    try:
        ...
        print(f"  Today so far:  {len(calls):>3} GO_CALL   {len(puts):>3} GO_PUT   "
              f"total={len(rows)}   last: {_ago(last_ts)}")
        print("  " + "─" * 96)
        print(f"  {'TIME':<13} {'DIR':<9} {'PROB':>6}  "
              f"{'UP':>7}  {'DN':>7}  {'ATM':>7}  "
        ...
        if not tail:
            ...
            for r in tail:
                ...
                tag = "\033[32mGO_CALL \033[0m" if direction == "GO_CALL" \
                    else "\033[31mGO_PUT  \033[0m"
                print(f"  {ts:<13} {tag} {_fmt(prob, 6, 3)}  "
