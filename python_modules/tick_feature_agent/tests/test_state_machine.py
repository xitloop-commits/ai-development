...
class TestCallback:
    def test_callback_receives_old_new_reason(self):
        ...
        sm.on_feed_reconnect_tick()
        assert events[0] == (TradingState.FEED_STALE, TradingState.WARMING_UP, "feed_reconnect_first_tick")
...
class TestTradingAllowedInvariant:
    ...
    def test_invariant_across_all_transitions(self):
        sm, _ = _sm()
        self._check(sm)
        sm.on_feed_reconnect_tick();  self._check(sm)
        sm.on_warm_up_complete();     self._check(sm)
        sm.on_chain_stale();          self._check(sm)
        sm.on_chain_recovered();      self._check(sm)
        sm.on_feed_disconnect();      self._check(sm)
        sm.on_feed_reconnect_tick();  self._check(sm)
        sm.on_warm_up_complete();     self._check(sm)
        sm.on_expiry_rollover();      self._check(sm)
