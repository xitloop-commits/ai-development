...
def _decide_via_gate(predictions: dict,
                     thresholds: Thresholds,
                     ce_ltp: float | None,
                     pe_ltp: float | None) -> SignalAction:
...
def run(instrument: str,
    ...
    if filter_mode == "gate":
        ...
        print(f"  Filter: 3-condition gate  "
    ...
    try:
        for line in _tail(live_path):
            ...
            if filter_mode == "gate":
                ...
                if np.isnan(up_swing): up_swing = preds["max_upside_300s"]
                if np.isnan(dn_swing): dn_swing = preds["max_drawdown_300s"]

                legacy = legacy_filter.legacy_decide(
                    preds["direction_prob_30s"],
                    preds["max_upside_30s"], preds["max_drawdown_30s"],
                    regime, ce_ltp, pe_ltp,
                    up_pred_swing=up_swing, dn_pred_swing=dn_swing,
                )
            ...
            if filter_mode == "gate":
                ...
                if rec is not None:
                    ...
                    print(f"  [{ts_short}] [TRADE-LEGACY] {rec.action:<10}  "
...
def main() -> int:
    ...
    p.add_argument("--config-dir", default="config/sea_thresholds",
                   help="Per-instrument JSON thresholds dir (default config/sea_thresholds)")
    p.add_argument("--filter", choices=("gate", "legacy"), default="gate",
